...
const doesParamsCircleExist = async (req: Request, res: Response, next: NextFunction) => {
  const circle = await CircleCollection.findByName(req.params.circleName);
  if(circle != null){
      next();
      return;
  }
